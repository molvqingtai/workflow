...
import { Worker, type ExecutionContext, type NextFunction, ExecutionStatus } from 'workflow'
...
class FailingWorker extends Worker {
  ...
  async execute(_context: ExecutionContext, _next: NextFunction): Promise<void> {
    // 执行失败：不调用 next，直接返回
    return
  }
}
...
class ContextDependentWorker extends Worker {
  ...
  async execute(context: ExecutionContext, next: NextFunction): Promise<void> {
    // 查找历史中的特定结果
    const previousResult = context.history.find((r) => r.workerId === 'simple-worker')
